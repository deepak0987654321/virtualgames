...
export default function CategoriesRoom() {
    ...
    const { user } = useAuth();
    const { roomCode, sessionId, name, audioOn, videoOn } = router.query;
        ...
        if (!name && user) {
            ...
            router.replace({
                ...
                query: {
                    ...
                    name: user.username,
                    avatar: myAvatar,
                    playerId: user.id || user.playerId
                }
            });
        }
    ...
    useEffect(() => {
        ...
        // Force Uppercase Room Code to fix invite link casing issues
        if (typeof roomCode === 'string' && roomCode !== roomCode.toUpperCase()) {
            router.replace({
                pathname: `/categories/room/${roomCode.toUpperCase()}`,
                query: { ...router.query, roomCode: roomCode.toUpperCase() }
            });
            return;
        }
        ...
        socket.on('categories:round_ended', ({ scores, answers }: any) => {
            ...
            setRoundAnswers(answers);
            setGameStatus('waiting'); // Return to lobby state for next round start
        });
    ...
    const isHost = room.players.find((p: any) => p.name === name || (p.playerId && p.playerId === (router.query.playerId as string)))?.isHost;
        ...
        <GameLayout
            ...
            renderVideo={(props) => (
                <div style={{ padding: '10px', textAlign: 'center', opacity: 0.5, fontSize: '0.8rem' }}>
                    ...
                    {/* Peer videos would be rendered here via GameLayout's system */}
                </div>
            )}
        >
                ...
                {gameStatus === 'finished' && (
                    <div className="leaderboard-phase animate-slide-up" style={{ maxWidth: '800px', margin: '0 auto' }}>
                        <div className="glass-panel" style={{ padding: '50px', textAlign: 'center' }}>
                            ...
                            <button onClick={() => router.push('/categories')} className="btn-ghost btn-lg" style={{ marginTop: '40px', width: '100%' }}>
                                Play Again
                            </button>
                        </div>
                    </div>
